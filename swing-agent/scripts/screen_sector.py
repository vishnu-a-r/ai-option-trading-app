...
from src.data.nse_source import NsePriceSource, NseUniverse, load_cache
...
def main() -> int:
    ...
    candidates = [
        Candidate(
            symbol=s,
            components={"technical_setup": technical_score(r), "relative_strength": 0.0},
            entry=r.entry,
            stop=stop,
            reason=r.reason,
        )
        for s, r, _, stop in passes
    ]
    ranked = rank(candidates, cfg)[: args.top]

    print(f"TOP {len(ranked)} BY TECHNICAL SETUP  (technical only - see module docstring)\n")
    hdr = f"{'#':<3}{'SYMBOL':<12}{'COMPANY':<32}{'ENTRY':>9}{'STOP':>9}{'RISK/SH':>9}{'R%':>7}{'RSI':>6}{'COV':>6}"
    print(hdr)
    print("-" * len(hdr))
    ...
    for i, c in enumerate(ranked, 1):
        ...
        risk = c.entry - c.stop
        print(
            f"{i:<3}{c.symbol:<12}{names.get(c.symbol,'')[:30]:<32}"
            f"{c.entry:>9.2f}{c.stop:>9.2f}{risk:>9.2f}{100*risk/c.entry:>6.1f}%"
            f"{r14:>6.0f}{c.coverage:>6.2f}"
        )
